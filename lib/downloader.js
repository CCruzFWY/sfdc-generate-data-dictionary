...
module.exports = class Downloader {
  ...
  execute() {
    const promise = new Promise((resolve, reject) => {
      ...
      // Download describe for all objects (both custom and standard)
      for (let object of self.config.objects) {
        downloadArray.push(self.downloadDescribe(object));
      }

      // Filter only custom objects for metadata download (standard objects don't have CustomObject metadata)
      let customObjects = self.config.objects.filter(obj => {
        return obj.indexOf('__c') !== -1;
      });

      // Only download metadata for custom objects
      if (customObjects.length > 0) {
        let loop = ~~(customObjects.length / 10);
        if (customObjects.length % 10 > 0)
          loop++;

        let j = 0;
        for (let i = 0; i < loop; i++) {
          let objectList = customObjects.slice(j, j + 10);
          j += 10;
      ...
      Promise.all(
        ...
        self.logger('Error downloading: ' + err);
        reject(err);
      });
    });
    return promise;
