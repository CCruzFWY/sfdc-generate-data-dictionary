...
const MAX_PICKLIST_VALUES = 10000;

/**
 * Sanitizes a sheet name by removing invalid characters and limiting length
 * @param {string} raw - The raw sheet name
...
function sanitizeSheetName(raw) {
  ...
  // Limit to 31 characters (Excel's maximum sheet name length)
...
// Track created sheet names to ensure uniqueness
const createdSheetNames = new Set();

/**
 * Ensures a unique sheet name by appending suffixes if necessary
 * @param {string} baseName - The base sheet name
 * @returns {string} - A unique sheet name
 */
function ensureUniqueSheetName(baseName) {
  let sheetName = baseName;
  let counter = 1;
  
  // Check if the sheet name already exists in our tracking set
  while (createdSheetNames.has(sheetName)) {
    // Calculate available space for suffix (e.g., "_1", "_2", etc.)
    ...
    if (counter > 1000) {
      sheetName = baseName.substring(0, 27) + '_' + Date.now();
      break;
    }
  }
  
  // Add the final unique name to our tracking set
  ...
  generate() {
    const promise = new Promise((resolve, reject) => {
      ...
      for (var i = 0; i < sObjects.length; i++) {
        ...
        const sanitized = sanitizeSheetName(sObjects[i]);
        const sheetName = ensureUniqueSheetName(sanitized);
        ...
        if (fs.existsSync(describePath)) {
          ...
          var currentObjectFieldsMetadata = null;
          var fieldsMap = null;

          if (fs.existsSync(metadataPath)) {
            currentObjectFieldsMetadata = JSON.parse(fs.readFileSync(metadataPath));
            if(currentObjectFieldsMetadata.fields != null){
              fieldsMap = this.mapFields(currentObjectFieldsMetadata.fields);
            }
          }
          ...
          for (var j = 0; j < currentObjectFieldsDescribe.length; j++) {
            ...
            if (fieldsMap != null && fieldsMap[fieldName] != null) {
          ...
          if (this.config.debug) {
            var validationRules = (currentObjectFieldsMetadata && currentObjectFieldsMetadata.validationRules) ? currentObjectFieldsMetadata.validationRules : null;
            this.utils.log('#' + sObjects[i] + '\n#Validation RULES ' + JSON.stringify(validationRules), this.config);
          }

          var validationRules = (currentObjectFieldsMetadata && currentObjectFieldsMetadata.validationRules) ? currentObjectFieldsMetadata.validationRules : undefined;
          this.writeFields(worksheet, currentObjectFieldsDescribe, line, validationRules);
